...
public class VuCompanyLocksRecord {
	...
	public VuCompanyLocksRecord(byte[] bytes) throws UnsupportedEncodingException{
		int start=0;
		this.lockInTime= RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.LOCKINTITME.getSize()));
		this.lockOutTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.LOCKOUTTIME.getSize()));
		Name n=new Name(Arrays.copyOfRange(bytes, start, start+=VuSizes.COMPANYNAME.getSize()));
		this.companyName=n.getName();
		n=new Name(Arrays.copyOfRange(bytes, start, start+=VuSizes.COMPANYADDRESS.getSize()));
		this.companyAddres=n.getName();
		this.companyCardNumber=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.FULLCARDNUMBER.getSize()));
