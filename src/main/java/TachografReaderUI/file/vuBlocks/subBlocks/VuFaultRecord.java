...
public class VuFaultRecord {
	...
	public VuFaultRecord(byte[] bytes) throws UnsupportedEncodingException {
		...
		start+=VuSizes.FAULTTYPE.getSize();
		this.faultRecordPurpose=EventFaultRecordPurpose.getEventFaultRecordPurpose(bytes[start]);
		start+=VuSizes.FAULTRECORDPURPOSE.getSize();
		this.faultBeginTime= RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.FAULTBEGINTIME.getSize()));
		this.faultEndTime=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.FAULTENDTIME.getSize()));
		FullCardNumber fcn;
		this.cardNumberDriverSlotBegin=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.CARDNUMBERDRIVERSLOTBEGIN_FAULT.getSize()));
		this.cardNumberCoDriverSlotBegin=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.CARDNUMBERCODRIVERSLOTBEGIN_FAULT.getSize()));
		this.cardNumberDriverSlotEnd=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.CARDNUMBERDRIVERSLOTEND_FAULT.getSize()));
		this.cardNumberCoDriverSlotEnd=new FullCardNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.CARDNUMBERCODRIVERSLOTEND_FAULT.getSize()));
	}
