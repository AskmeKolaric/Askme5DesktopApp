...
public class VuOverSpeedingControlData {
	...
	public VuOverSpeedingControlData(byte[] bytes) {
		int start=0;
		this.lastOverspeedControlTime= RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.LASTOVERSPEEDCONTROLTIME.getSize()));
		this.firstOverspeedSince=RealTime.getRealTime(Arrays.copyOfRange(bytes, start, start+=VuSizes.FIRSTOVERSPEEDSINCE.getSize()));
		this.numberOfOverspeedSince= Number.getNumber(Arrays.copyOfRange(bytes, start, start+=VuSizes.NUMBEROFOVERSPEEDSINCE.getSize()));
	}
