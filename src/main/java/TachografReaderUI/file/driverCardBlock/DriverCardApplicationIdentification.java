...
import TachografReaderUI.file.driverCardBlock.subBlocks.CardActivityLengthRange;
import TachografReaderUI.file.driverCardBlock.subBlocks.CardStructureVersion;
import TachografReaderUI.file.driverCardBlock.subBlocks.NoOfCardPlaceRecords;
import TachografReaderUI.file.driverCardBlock.subBlocks.NoOfCardVehicleRecords;
import TachografReaderUI.file.driverCardBlock.subBlocks.NoOfEventsPerType;
import TachografReaderUI.file.driverCardBlock.subBlocks.NoOfFaultsPerType;
...
public class DriverCardApplicationIdentification extends Block
	...
	public DriverCardApplicationIdentification(byte[] datos){
		int start=0;
		this.typeOfTachographCardId=new EquipmentType(Arrays.copyOfRange(datos, start, start+= DriverCardSizes.TYPEOFTACHOGRAPHCARDID.getMax()));
		this.cardStructureVersion=new CardStructureVersion(Arrays.copyOfRange(datos, start, start+= DriverCardSizes.CARDSTRUCTUREVERSION.getMax()));
		this.noOfEventsPerType=new NoOfEventsPerType(Arrays.copyOfRange(datos, start, start+= DriverCardSizes.NOOFEVENTSPERTYPE.getMax()));
		this.noOfFaultsPerType=new NoOfFaultsPerType(Arrays.copyOfRange(datos,start,start+= DriverCardSizes.NOOFFAULTSPERTYPE.getMax()));
		this.activityStructureLength=new CardActivityLengthRange(Arrays.copyOfRange(datos, start, start+= DriverCardSizes.ACTIVITYSTRUCTURELENGTH.getMax()));
		this.noOfCardVehicleRecords=new NoOfCardVehicleRecords(Arrays.copyOfRange(datos, start, start+= DriverCardSizes.NOOFCARDVEHICLERECORDS.getMax()));
		this.noOfCardPlaceRecords=new NoOfCardPlaceRecords(Arrays.copyOfRange(datos,start, start+= DriverCardSizes.NOOFCARDPLACERECORDS.getMax()));
	}
