...
import TachografReaderUI.file.driverCardBlock.subBlocks.CardFaultRecord;

/**
 * 2.17. CardFaultData
 * Informaci�n almacenada en una tarjeta de conductor o en una tarjeta del centro de ensayo y relativa a los fallos asociados
 ...
 * 
 * cardFaultRecords es un conjunto de registros de fallos de una categor�a determinada (del aparato de control o de la
 * tarjeta).
 * 
...
public class CardFaultData extends Block implements CardBlock {
	...
	public CardFaultData(byte[] datos) {
		...
		for (int i=0;i<datos.length;i+= DriverCardSizes.CARDFAULTRECORD.getMax()){
			CardFaultRecord cer=new CardFaultRecord(Arrays.copyOfRange(datos,i,start+= DriverCardSizes.CARDFAULTRECORD.getMax()));
			this.cardFaultRecords.add(cer);
		}				
	}

	/**
	 * Obtiene un conjunto de registros de fallos de una categor�a determinada (del aparato de control o de la
	 *	tarjeta).
	...
	/**
	 * Asigna un conjunto de registros de fallos de una categor�a determinada (del aparato de control o de la
	 * tarjeta).
